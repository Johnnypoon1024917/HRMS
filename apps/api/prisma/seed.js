...
const registry_1 = require("../node_modules/.prisma/registry");
...
async function main() {
    const registry = new registry_1.PrismaClient();
    ...
    (0, node_child_process_1.execSync)(`npx prisma db push --schema prisma/tenant.prisma --skip-generate --accept-data-loss`, { env: { ...process.env, DATABASE_URL: `${BASE}?schema=${SCHEMA}` }, stdio: 'inherit' });
 ...
 */
async function seedDemoStaff(db) {
    ...
    if (!hq)
        throw new Error('HQ org unit missing — base seed did not run.');
    ...
    for (let i = 2; i <= TARGET + 1; i++) {
        ...
        if (await db.staff.findUnique({ where: { staffNo } }))
            continue;
        const isMale = i % 2 === 0;
        const nameEn = isMale
